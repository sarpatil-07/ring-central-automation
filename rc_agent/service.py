"""macOS LaunchAgent for RC Agent API scheduler."""
...
LABEL = "com.ringcentralautoset.rc-api.scheduler"
...
def _run_py() -> Path:
    return config.BASE_DIR / "rc_run.py"
...
def build_plist() -> dict:
    ...
    return {
        ...
        "RunAtLoad": True,
        "KeepAlive": True,
        "StandardOutPath": str(LOG_DIR / "rc-api-scheduler.log"),
        "StandardErrorPath": str(LOG_DIR / "rc-api-scheduler.err.log"),
...
def install() -> str:
    ...
    return (
        f"Installed {config.RC_APP_NAME} background scheduler.\n"
        f"  Plist: {path}\n"
        f"  Logs:  {LOG_DIR}/rc-api-scheduler.log\n"
        f"Uses Agent API — keep desktop MAX open for calls."
    )
...
def uninstall() -> str:
    ...
    if path.exists():
        path.unlink()
    return f"{config.RC_APP_NAME} background scheduler removed."


def status() -> str:
    path = plist_path()
    if not path.exists():
        return "Not installed. Run: .venv/bin/python rc_run.py install-service"
    result = subprocess.run(
